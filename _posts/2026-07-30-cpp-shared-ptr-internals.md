...
| 层面 | 线程安全？ | 说明 |
|------|-----------|------|
| 控制块的引用计数 | ✅ | 原子操作，多线程各持副本时增减互不干扰 |
| shared_ptr 变量本身 | ❌ | 多个线程同时读写同一个变量是 data race |
| 被管理的对象 | ❌ | 只管理生命周期，不管内部状态同步 |

⭐ **关键结论**：按下面规范使用时——每个线程持有独立 `shared_ptr` 副本、不共享同一个变量——引用计数的增减和资源的申请/释放**都是线程安全的**，由原子操作保证。但一旦违反规范（例如引用捕获 `shared_ptr` 并跨线程读写同一变量），结果就是 **UB**，没有任何保证。
